...
export default async function DashboardPage() {
  ...
  const ordersCount = summary?.ordersCount?.[0]?.count || 0
  const productsCount = summary?.productsCount?.[0]?.count || 0
  const usersCount = summary?.usersCount?.[0]?.count || 0
  const totalRevenue = summary?.ordersPrice?.[0]?.sum || 0
