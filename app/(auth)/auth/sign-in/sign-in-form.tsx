...
export default function SignInForm({ error, callbackUrl }: SignInFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    console.log('🔍 Form: Preventing default and starting validation...')
    if (!validateForm()) {
    ...
    try {
      console.log('🔐 Form: Attempting login with data:', { email: formData.email, password: '***' })
      ...
      if (!result.success) {
        console.log('❌ Form: Login failed:', result.error)
        setErrors({ email: result.error || 'Login failed' })
        setIsLoading(false)
      } else {
        console.log('✅ Form: Login successful!', result.user)
        alert('🎉 Login Successful! Redirecting...')
        
        // Store user info in localStorage
        localStorage.setItem('user', JSON.stringify(result.user))
        console.log('💾 Form: User stored in localStorage')
        
        // Don't set loading to false - keep spinner until redirect
        console.log('🚀 Form: Keeping loading state for redirect...')
        
        // Force immediate redirect
        if (result.user.role === 'admin') {
          console.log('🔄 Form: Admin user - FORCING redirect to /admin...')
          window.location.href = '/admin'
        } else {
          console.log('🔄 Form: Regular user - FORCING redirect to home...')
  ...
  const handleGoogleSignIn = async () => {
    ...
    setErrors({ email: 'Google sign-in temporarily disabled. Use email/password.' })
  }
